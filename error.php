                ...
                <div class="container mx-auto px-6 flex flex-col-reverse lg:flex-row justify-between items-center relative">
                    <div class="w-full mb-16 md:mb-8 text-center lg:text-left">
                        <h1 class="text-center lg:text-left text-5xl lg:text-8xl mt-12 md:mt-0 text-gray-700">
                            Error: <?php
                                if(isset($_GET['error'])) {
                                    if($_GET['error'] == 'db_conn') { echo 'Database Error.'; }
                                    else if($_GET['error'] == '401') { echo 'Unauthorized.'; }
                                    else if($_GET['error'] == '403') { echo 'Access Denied.'; }
                                    else if($_GET['error'] == '404') { echo 'Page not Found.'; }
                                    else if($_GET['error'] == '405') { echo 'Method not Allowed.'; }
                                    else if($_GET['error'] == '408') { echo 'Request Timeout.'; }
                                    else if($_GET['error'] == '413') { echo 'Payload Too Large.'; }
                                    else if($_GET['error'] == '414') { echo 'URI Too Long.'; }
                                    else if($_GET['error'] == '425') { echo 'Too Early.'; }
                                    else if($_GET['error'] == '429') { echo 'Too Many Requests.'; }
