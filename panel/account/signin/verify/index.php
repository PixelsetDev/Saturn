    ...
    if(isset($_POST['verify'])) {
        if(!empty($_POST['code'])) {
            ...
            if ($_POST['code'] == $dbCode) {
                ...
                log_file('SATURN][SECURITY','Failed login verify attempt by user with IP: '.htmlspecialchars($_SERVER['REMOTE_ADDR']));
                    ...
                    <form class="mt-8 space-y-6" action="<?php echo htmlspecialchars($_SERVER['PHP_SELF'])?>?username=<?php echo checkOutput('DEFAULT', $username);?>" method="post">
                        ...
                        <div class="rounded-md shadow-sm -space-y-px">
                            <div>
                                ...
                                <input id="code" name="code" type="code" autocomplete="code" required class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm" placeholder="Verification Code">
                            </div>
                        </div>
