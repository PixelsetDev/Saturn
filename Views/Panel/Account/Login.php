    ...
    <body>
        ...
        <div class="absolute top-0 bottom-0 left-0 right-0 md:filter-none bg-white/50 md:mx-auto md:w-1/2 w-full mx-2 z-20 lg:my-16 md:my-8 sm:my-4 my-2 rounded-md shadow-lg">
            <form class="lg:px-8 lg:py-10 md:px-6 md:py-8 sm:px-4 sm:py-6 px-2 py-4 text-white relative w-full h-full" action="/panel" method="POST">
                ...
                <input type="text" name="username" id="username" maxlength="63" required placeholder="<?= $TL->TL('UsernameEmail'); ?>" class="block w-full px-2 py-1 rounded-t-md bg-neutral-100 text-black appearance-none border">
                <input type="password" name="password" id="password" maxlength="63" required placeholder="<?= $TL->TL('Password'); ?>" class="block w-full px-2 py-1 rounded-b-md bg-neutral-100 text-black appearance-none mb-8 border">
