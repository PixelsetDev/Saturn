    ...
    if ($themeData->{'theme'}->{'name'} != null || $themeData->{'theme'}->{'name'} != '') {
        if (CONFIG_DEBUG) {
            if ($themeData->{'theme'}->{'version'}->{'saturn'} != SATURN_VERSION) {
                log_console('SATURN][RERL][THEMES', 'WARNING: Theme '.$themeData->{'theme'}->{'name'}.' is for Saturn version '.$themeData->{'theme'}->{'version'}->{'saturn'}.' - you are running version '.SATURN_VERSION);
            }
            log_console('SATURN][RERL][THEMES', 'Loaded theme: '.$themeData->{'theme'}->{'name'});
        }
    } else {
        if (CONFIG_DEBUG) {
            log_console('SATURN][RERL][THEMES', 'Unable to load theme: '.THEME_SLUG);
        }
